...
class Stusmview extends Controller{
	...
	function findstudymaterial(){
        ...
        if($res == ""){
            //Logdebug::appendlog('$res');
            $material =  $this->model->findstudymaterial($subject,$student);
            Logdebug::appendlog(print_r($material, true));
	...
	function script(){
	   ...
	   $('#clearall').on('click', function(){
		...
		var subjects = '".URL."studymaterialview/fillSubject';
		...
		$('#searchnotice').on('click', function(){
			...
			var url = '".URL."studymaterialview/findstudymaterial';
			//console.log(url);
					...
					$.ajax({
						...
						if(!result.result){
						   $.each(result, function(key, value){
								crow = crow + 1;
								tblhtml+='<div class=\"col-lg-6\" style=\"margin-top: 5px\"><div class=\"card\"><div class=\"card-header\"><h4 class=\"card-title\">'+value.xsubname+'</h4><p class=\"text-muted mb-0\">Date : '+value.xdate+'</p></div><div class=\"card-body\"><div class=\"embed-responsive embed-responsive-16by9\"><iframe class=\"embed-responsive-item\" src=\"'+value.xemburl+'\" allowfullscreen></iframe></div></div></div></div>';      
								if(crow % 2 == 0){
									tblhtml+='</div>';
									tblhtml+='<div class=\"row\">';
								}
