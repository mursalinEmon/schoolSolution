...
class Crmwp extends Controller{
	...
	function init(){
		...
		echo "CRM Work Progress Started...";
		
	}
	
	function createworkprogress(){
		...
		for($i=0; $i<count($dataarray); $i++){
			if($dataarray[$i]["menu"]==="Customer Work Progress"){
		...
		$adddata = array(			
			"xwsl"=>$data->progressid,
			"zemail"=>$roldet[0]["zemail"],
			"bizid" => $roldet[0]["bizid"],	
			"xdate"=>$date,		
			"xprogresstype"=>$data->progresstype,
			"xprogresspct"=>$data->progresspct,
			"xquotnum"=>$data->quotnum,
			"xlead"=>$data->leadid,			
		...
		xprogresstype='".$data->progresstype."', xprogresspct='".$data->progresspct."',xquotnum='".$data->quotnum."',
		xlead='".$data->leadid."',xfeedback='".$data->feedback."',		
		xremarks='".$data->remarks."'";

		
		
		$result = $this->model->createwp($adddata,$onduplicate);
	...
	function findquot($token,$quot=""){
		$roldet = $this->model->getroledt($token);
		if(count($roldet)===0){
			echo json_encode(array('result'=>'error','response'=>'Token Mismatch!','message'=>''));
			exit;
		}
		$this->model->getquotbycode($quot, $roldet[0]["bizid"]);
	}
	function progresslist($token,$quotnum){
		$roldet = $this->model->getroledt($token);
		if(count($roldet)===0){
			echo json_encode(array('result'=>'error','response'=>'Token Mismatch!','message'=>''));
			exit;
		}
		$this->model->getprogresslist($quotnum,$roldet[0]["bizid"]);
	}
