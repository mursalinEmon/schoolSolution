...
class Stusalesconf extends Controller{
	...
	private function intializeform(){
        ...
        $this->formset = array(
            ...
            "actionbtn"=>array(                
                array("btnmethod"=>"update","btntext"=>"Update","btnurl"=>URL."confirmsale/updatenotice","btnid"=>"noticeupdate"),
            ),
            "mainbtn"=>array(
                array("btnmethod"=>"save","btntext"=>"Save Notice","btnurl"=>URL."confirmsale/savenotice","btnid"=>"usersave", "icon"=>"<i class=\"far fa-save mr-1\"></i>","btncolor"=>"btn-primary"),
                array("btnmethod"=>"view","btntext"=>"View Notice","btnurl"=>URL."confirmsale/singlenotice","btnid"=>"viewnotice", "icon"=>"<i class=\"far fa-info mr-1\"></i>","btncolor"=>"btn-info"),
        ...
        $this->searchsettings = array(
            ...
            "actionbtn"=>array(),
            "mainbtn"=>array(                
                array("btnmethod"=>"search","btntext"=>"Search","btnurl"=>URL."confirmsale/findnotice","btnid"=>"searchnotice", "icon"=>"<i class=\"fa fa-eraser mr-1\"></i>","btncolor"=>"btn-success"),
            ),
        );
	...
	function findnotice(){
        $res = "";
		$conditions = "bizid = ".Session::get('sbizid')." and xstatus = 'Created'";
        $mobile = "";
        $itemcode = "";

		if(!empty($_POST['itmcode']))
		...
		if($itemcode != ""){
			$conditions .=" and xitemcode = '".$itemcode."'";
		}

		if($mobile != ""){
			$conditions .=" and xstudentmobile like '%".$mobile."%'";
		}
...
Hotline: ".Session::get('sbizmobile')."";
	...
	//Logdebug::appendlog($result);
        if($result)
            echo json_encode(array('message'=>'Sales confirm Successfully','result'=>'success','keycode'=>$result));
        else
	...
	function script(){
	   ...
	   $('#clearall').on('click', function(){
		...
		$('#searchnotice').on('click', function(){
			...
			var url = '".URL."confirmsale/findnotice';
			var formid = 'frmsearch';
					...
					$.ajax({
						...
						if(!result.result){
							tblhtml='<thead><th>Date</th><th>Student ID</th><th>Name</th><th>Course</th><th>Mobile</th><th>Paid By</th><th>Txn ID</th><th>Txn Mobile</th><th>Action</th></thead>';
							tblhtml+='<tbody>';
							$.each(result, function(key, value){
							
								 tblhtml+='<tr id=\"tem'+value.xtemsl+'\"><td>'+value.xdate+'</td><td>'+value.xstudent+'</td><td>'+value.xstuname+'</td><td>'+value.xitemdesc+'</td><td>'+value.xstudentmobile+'</td><td>'+value.xoperator+'</td><td>'+value.xtxnnum+'</td><td>'+value.xtxnmobile+'</td><td><a class=\"btn btn-success\" style=\"border-radius:60px; font-size: 12px; padding: 5px 5px;\" data-toggle=\"modal\" data-target=\"#exampleModal\" onClick=\"open_modal(\''+value.xtemsl+'\',\'Confirm\')\">Confirm</a><a class=\"btn btn-danger\" style=\"border-radius:60px; font-size: 12px; padding: 5px 5px;\" data-toggle=\"modal\" data-target=\"#exampleModal\" onClick=\"open_modal(\''+value.xtemsl+'\',\'Delete\')\">Delete</a></td></tr>';
        ...
        $('#btnbatch').on('click', function(){
            var url = '".URL."confirmsale/saveAssign';
            var uid = $('#tempsl').val();
            var txnid = $('#txnid').val();
		...
		var courses = '".URL."confirmsale/getCourse';
		//console.log(courses);
