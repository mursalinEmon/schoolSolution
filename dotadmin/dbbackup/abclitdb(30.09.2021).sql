...
(1, 100, '2021-09-27 12:00:16', 'rajib@dbs.com', NULL, NULL, 8, '2021-09-27', 0, '4', 4, 'fdbd', 0),
...
CREATE TABLE `userbase` (
  ...
  `user_created_unix` date DEFAULT current_timestamp(),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vcustomerbal`  AS SELECT `gldetail`.`bizid` AS `bizid`, `gldetail`.`xaccsub` AS `xaccsub`, sum(`gldetail`.`xprime`) AS `xbal` FROM `gldetail` WHERE `gldetail`.`xaccsource` = 'Customer' GROUP BY `gldetail`.`bizid`, `gldetail`.`xaccsub` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vitemcost`  AS SELECT `vitemcostdt`.`bizid` AS `bizid`, `vitemcostdt`.`xitemcode` AS `xitemcode`, `vitemcostdt`.`totalqty` AS `totalqty`, `vitemcostdt`.`totalpurchasecost`/ `vitemcostdt`.`totalqty` AS `avgcost` FROM `vitemcostdt` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vitemcostdt`  AS SELECT `vimtrn`.`bizid` AS `bizid`, `vimtrn`.`xitemcode` AS `xitemcode`, sum(`vimtrn`.`xqty` * `vimtrn`.`xsign`) AS `totalqty`, sum(`vimtrn`.`xqty` * `vimtrn`.`xstdcost` * `vimtrn`.`xsign`) AS `totalpurchasecost` FROM `vimtrn` WHERE `vimtrn`.`xqty` <> 0 GROUP BY `vimtrn`.`bizid`, `vimtrn`.`xitemcode` ORDER BY `vimtrn`.`xitemcode` ASC ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vrecnpayvou`  AS SELECT DISTINCT `gldetailview`.`bizid` AS `bizid`, `gldetailview`.`xvoucher` AS `xvoucher` FROM `gldetailview` WHERE `gldetailview`.`xaccusage` in ('Bank','Cash') ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vsecodes`  AS SELECT `secodes`.`bizid` AS `bizid`, `secodes`.`codeid` AS `codeid`, `secodes`.`xcodetype` AS `xcodetype`, `secodes`.`xcode` AS `xcode`, `secodes`.`xdesc` AS `xdesc`, `secodes`.`xcoderem` AS `xcoderem` FROM `secodes` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vsomilestone`  AS SELECT `m`.`bizid` AS `bizid`, `m`.`ztime` AS `ztime`, `m`.`zemail` AS `zemail`, `m`.`xsonum` AS `xsonum`, `m`.`xquotnum` AS `xquotnum`, `m`.`xdate` AS `xdate`, `m`.`xcus` AS `xcus`, (select coalesce(count(0),0) from `imreqdelmst` `c` where `m`.`xsonum` = `c`.`xsonum` and `m`.`bizid` = `c`.`bizid`) AS `xdelcount` FROM `somst` AS `m` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vsupplierbal`  AS SELECT `gldetail`.`bizid` AS `bizid`, `gldetail`.`xaccsub` AS `xaccsub`, sum(`gldetail`.`xprime`) AS `xbal` FROM `gldetail` WHERE `gldetail`.`xaccsource` = 'Supplier' GROUP BY `gldetail`.`bizid`, `gldetail`.`xaccsub` ;
